...
//        return arrayList.get(0).getData().getPosts().size();
    ...
    @Override
    public int getCount() {
        return oneBean.getData().getPosts().size();
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        viewHdler.tv_title_long.setText(oneBean.getData().getPosts().get(position).getTitle());
        if (oneBean.getData().getPosts().get(position).getColumn() == null) {
            viewHdler.tv_title_short.setText("");
            viewHdler.tv_category.setText("");
        } else {
            viewHdler.tv_title_short.setText(oneBean.getData().getPosts().get(position).getColumn().getTitle());
            viewHdler.tv_category.setText(oneBean.getData().getPosts().get(position).getColumn().getCategory());
        }
        viewHdler.tv_nickname.setText(oneBean.getData().getPosts().get(position).getAuthor().getNickname());
        viewHdler.tv_likes_count.setText(oneBean.getData().getPosts().get(position).getLikes_count() + "");
        Picasso.with(context).load(oneBean.getData().getPosts().get(position).getAuthor().getAvatar_url()).into(iv_avatar_url);
        Picasso.with(context).load(oneBean.getData().getPosts().get(position).getCover_image_url()).into(iv_cover_image_url);
        viewHdler.ll.setOnClickListener(new View.OnClickListener() {
            @Override
